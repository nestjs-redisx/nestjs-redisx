 ...
 * default Express adapter. Mirrors fastify.e2e-spec.ts to catch divergence
 * between the two HTTP adapters.
 */
describe('Rate limit + idempotency (Express)', () => {
  ...
  it('returns 429 with structured body when limit is exceeded', async () => {
    // Drain remaining quota then trigger the overflow. The previous test already
    // consumed one point, so 10 additional requests push us over the window.
    for (let i = 0; i < 10; i++) {
  ...
  it('errors when the same key is reused with a different body', async () => {
    const key = `e2e-express-${Date.now()}-mismatch`;
